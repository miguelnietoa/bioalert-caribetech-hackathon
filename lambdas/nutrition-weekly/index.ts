...
const STUDENT_SQL = `
-- Top productos + macros últimos 7 días, SOLO del hijo principal del padre.
-- Multi-hijo aware + peso por unidad (fallback 50g si Claude no estimó).
WITH yo AS (
...
-- Macros con peso por unidad (fallback 50g).
WITH last7 AS (
  SELECT v.*, pn.calories_100g, pn.sugar_g, pn.fat_g, pn.sodium_mg,
         COALESCE(pn.gramos_por_unidad, 50) AS gramos
  FROM reto.ventas v
  ...
  for (const p of parents) {
    try {
      ...
      const body =
        ...
        `¿Quieres ver opciones de recarga que prioricen lo saludable? Responde *Recargar*.`
